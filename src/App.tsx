...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-50 flex flex-col font-sans">
      {/* Header & Search */}
      <header className="p-6 border-b border-zinc-800 bg-zinc-900/50 sticky top-0 z-10 backdrop-blur-md">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex bg-zinc-800/50 rounded-lg p-1">
              <button 
                ...
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'search' ? 'bg-zinc-700 text-white' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                Search
              </button>
              <button 
                onClick={() => setActiveTab('playlists')}
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'playlists' ? 'bg-zinc-700 text-white' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                Playlists
              </button>
              <button 
                onClick={() => { setActiveTab('history'); setActivePlaylist(null); }}
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'history' ? 'bg-zinc-700 text-white' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                History
              </button>
              <button 
                onClick={() => { setActiveTab('downloads'); setActivePlaylist(null); }}
                className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === 'downloads' ? 'bg-zinc-700 text-white' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                Downloads
              </button>
            </div>
          ...
          {activeTab === 'search' && (
            <form onSubmit={handleSearch} className="flex-1 w-full relative" role="search" aria-label="Search music">
              <input
                ...
                className="w-full bg-zinc-800/50 border border-zinc-700 rounded-full py-3 px-5 pl-12 text-zinc-100 placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
              />
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-zinc-400" aria-hidden="true" />
              <button 
                type="submit" 
                aria-label="Submit search"
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 p-2 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-zinc-900"
      ...
      {/* Main Content */}
      <main className="flex-1 max-w-4xl w-full mx-auto p-6 pb-32 relative">
        {isLyricsOpen && currentTrack ? (
          <div className="absolute inset-0 bg-zinc-950/95 backdrop-blur-xl z-20 p-6 sm:p-12 overflow-y-auto rounded-xl border border-zinc-800/50 m-6 mb-32 flex flex-col">
            <div className="flex justify-between items-center mb-8 sticky top-0 bg-zinc-950/90 py-4 backdrop-blur-md z-10 border-b border-zinc-800/50">
              <div>
                <h2 className="text-2xl font-bold text-zinc-100">{currentTrack.title}</h2>
                <p className="text-zinc-400">{currentTrack.user}</p>
              </div>
              <button 
                onClick={toggleLyrics}
                aria-label="Close lyrics"
                className="p-2 bg-zinc-800 hover:bg-zinc-700 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500"
              >
            ...
            <div className="flex-1 flex flex-col items-center justify-center min-h-[50vh]">
              {isLoadingLyrics ? (
                ...
                <div className="w-full max-w-2xl mx-auto text-center sm:text-left">
                  <p className="whitespace-pre-wrap text-lg sm:text-xl leading-relaxed text-zinc-300 font-medium">
                    {lyrics}
                  </p>
                </div>
              ) : null}
      ...
      {currentTrack && (
        <div className="fixed bottom-0 left-0 right-0 bg-zinc-900 border-t border-zinc-800 p-4 z-50">
          ...
          />
          <div className="max-w-4xl mx-auto flex items-center gap-4 sm:gap-6 relative z-10">
            <div className="flex items-center gap-3 w-1/3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="font-medium text-zinc-100 truncate text-sm">{currentTrack.title}</div>
                <div className="text-xs text-zinc-400 truncate">{currentTrack.user}</div>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col items-center gap-2">
              <div className="flex items-center gap-4">
                <button 
                  ...
                  className={`transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-zinc-900 rounded-full p-2 ${isShuffle ? 'text-emerald-400 bg-emerald-400/10' : 'text-zinc-400 hover:text-zinc-100'}`}
                >
                  <Shuffle className="w-4 h-4" aria-hidden="true" />
                </button>
                <button 
                  onClick={playPrevious}
                  disabled={queue.length === 0 || (queueIndex <= 0 && repeatMode !== 'all')}
                  aria-label="Previous track" 
                  className="text-zinc-400 hover:text-zinc-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-zinc-900 rounded-full p-1"
                >
                ...
                <button 
                  ...
                  className="w-10 h-10 flex items-center justify-center bg-white text-black rounded-full hover:scale-105 transition-transform focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-zinc-900"
                >
                  {isLoadingTrack ? (
                    ...
                    <Play className="w-5 h-5 fill-current ml-1" aria-hidden="true" />
                  )}
                </button>
                <button 
                  onClick={playNext}
                  disabled={queue.length === 0 || (queueIndex >= (isShuffle ? shuffledQueue.length : queue.length) - 1 && repeatMode !== 'all')}
                ...
                <button 
                  ...
                  className={`transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-zinc-900 rounded-full p-2 ${repeatMode !== 'off' ? 'text-emerald-400 bg-emerald-400/10' : 'text-zinc-400 hover:text-zinc-100'}`}
                >
                  {repeatMode === 'one' ? <Repeat1 className="w-4 h-4" aria-hidden="true" /> : <Repeat className="w-4 h-4" aria-hidden="true" />}
                </button>
              </div>
              <div className="w-full flex items-center gap-2 text-xs text-zinc-400">
                ...
                <input
                  ...
                  className="flex-1 h-1 bg-zinc-700 rounded-full appearance-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:rounded-full cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-zinc-900"
                />
            ...
            <div className="w-1/3 hidden sm:flex items-center justify-end gap-4">
              <button 
                ...
                className={`transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-zinc-900 rounded-full p-2 ${isLyricsOpen ? 'text-emerald-400 bg-emerald-400/10' : 'text-zinc-400 hover:text-zinc-100'}`}
              >
                <Mic2 className="w-5 h-5" aria-hidden="true" />
              </button>
              <div className="flex items-center gap-2">
                <Volume2 className="w-5 h-5 text-zinc-400" aria-hidden="true" />
                <input
                  type="range"
                  min={0}
                  max={1}
                  ...
                  aria-label="Volume"
                  className="w-16 md:w-24 h-1 bg-zinc-700 rounded-full appearance-none [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-3 [&::-webkit-slider-thumb]:h-3 [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:rounded-full cursor-pointer focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 focus:ring-offset-zinc-900"
                />
              </div>
            </div>
          </div>
